...
## 5. กรณีศึกษาในrepo นี้: `review-coverage` (prompt) vs `tc-audit` (skill)

สอง artifact นี้ดูเหมือน "ตรวจ TC" เหมือนกัน แต่ออกแบบเป็นคนละชนิด**โดยตั้งใจ** — เป็นตัวอย่างจริงของ prompt vs skill

### ต่างที่ "คำถามที่ตอบ"
| | `review-coverage` (Prompt) | `tc-audit` (Skill) |
|---|---|---|
| ตอบคำถาม | TC **ครบ**ทุก logic path มั้ย? | TC แต่ละแถว **ถูกกฎ/format** มั้ย? |
| มุมมอง | completeness — *"ขาดอะไรมั้ย"* | validity — *"ของที่มี ถูกมั้ย"* |
| ดูไฟล์ | `user-flow.md` ↔ `business-tc.md` (เทียบกัน) | `business-tc.md` ในตัวเอง |
| เจออะไร | logic path ที่ยังไม่มี TC + % coverage | label ผิด, TC-ID ซ้ำ, คอลัมน์เพี้ยน, ช่องว่าง |

> จับคู่กันได้: coverage บอก "ครบมั้ย" · audit บอก "ถูกมั้ย" — QA คนละด่าน ไม่ทับกัน

### ต่างที่ "กลไก"
| | `review-coverage` (Prompt) | `tc-audit` (Skill) |
|---|---|---|
| ชนิด | `.prompt.md` ไฟล์เดียว | folder: SKILL.md + scripts + references + assets |
| วิธีตรวจ | โมเดล **อ่านแล้วตีความเอง** | รัน **`check_tc.py`** ตรวจด้วยโค้ด |
| ความแม่น | ขึ้นกับโมเดล รันซ้ำอาจต่างนิด | **ตายตัว ซ้ำได้** ทุกครั้ง |
| เรียก | `/` manual เท่านั้น | `/` หรือ Agent หยิบเอง |

### กฎเลือกใช้ (lesson สำคัญ)
- งานที่ **"ถูก-ผิดชัดเจน"** (format, label ใน 3 ค่า, ซ้ำมั้ย) → **skill + script** ผลแม่นกว่าให้โมเดลเดา
- งานที่ **"ต้องใช้ดุลพินิจ"** (TC-05 ครอบคลุม LP-4 จริงมั้ย — semantic) → **prompt** ให้โมเดลคิด

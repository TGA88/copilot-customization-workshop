...
💡 (เชื่อมเข้า prompt vs skill) 🗣️ "สังเกตนะครับ `/review-coverage` ตอบว่า TC **ครบมั้ย** — งานแบบนี้ต้องใช้ดุลพินิจว่า TC ครอบคลุม path จริงไหม เลยเหมาะเป็น **prompt** ให้โมเดลคิด · แต่ถ้าถามว่า TC **ถูก format มั้ย** (label ใน 3 ค่า, TC-ID ซ้ำ) อันนั้นถูก-ผิดชัดเจน → เราทำเป็น **skill `tc-audit` ที่มี script ตรวจ** ผลแม่นกว่า (ดู Bonus Session 3)"
🗣️ "กฎง่าย ๆ: งานถูก-ผิดชัดเจน → skill+script · งานต้องใช้ดุลพินิจ → prompt"
